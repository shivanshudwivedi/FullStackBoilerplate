-- ============================================================================
-- QUICKSTART - Run this entire file in Supabase SQL Editor
-- ============================================================================
-- This combines schema.sql content for easy copy-paste setup
-- 
-- Instructions:
...
-- 6. Create your first admin user (see bottom of file)
-- ============================================================================

-- NOTE: For full schema with comments, use schema.sql
-- This is a condensed version for quick setup
...
create table if not exists candidate_assessments (
  ...
  start_token text,
  start_slug text unique not null,
...
create table if not exists events (
  ...
  type text not null check (type in (
    'assessment_created', 'invitation_sent', 'assessment_started', 'assessment_submitted',
    'assessment_expired', 'comment_added', 'comment_resolved', 'ranked',
    'ai_analysis_generated', 'follow_up_sent', 'repo_created', 'repo_archived',
...
create policy "users_select_own_profile" on profiles for select using (auth.uid() = user_id);
create policy "users_update_own_profile" on profiles for update using (auth.uid() = user_id);

create policy "admins_all_assessments" on assessments for all using (is_admin());
create policy "candidates_select_assigned_assessments" on assessments for select using (
  id in (select assessment_id from candidate_assessments where candidate_id = get_current_candidate_id())
);

create policy "admins_all_candidates" on candidates for all using (is_admin());
create policy "candidates_select_own_record" on candidates for select using (id = get_current_candidate_id());

create policy "admins_all_candidate_assessments" on candidate_assessments for all using (is_admin());
create policy "candidates_select_own_assessments" on candidate_assessments for select using (candidate_id = get_current_candidate_id());
create policy "candidates_update_own_assessments" on candidate_assessments for update using (candidate_id = get_current_candidate_id());

create policy "admins_all_repos" on repos for all using (is_admin());
create policy "candidates_select_own_repo" on repos for select using (
  candidate_assessment_id in (select id from candidate_assessments where candidate_id = get_current_candidate_id())
);

create policy "admins_all_reviews" on reviews for all using (is_admin());
create policy "admins_all_comments" on comments for all using (is_admin());
create policy "admins_all_events" on events for all using (is_admin());
create policy "candidates_select_own_events" on events for select using (
  candidate_assessment_id in (select id from candidate_assessments where candidate_id = get_current_candidate_id())
);
...
--    - Sign up via your app or Supabase Auth UI
